...
(define (composite  relationOuter relationInner)
  (if (null? relationOuter)
      '()
      (append(compHelp(car relationOuter)relationInner)
             (composite(cdr relationOuter)relationInner))))
(define(compHelp tuple inner)
  (if (null? inner)
      '()
      (if(equal?(cadar inner)(car tuple))
         (cons (list(caar inner)(cadr tuple)) (compHelp tuple(cdr inner)))
         (compHelp tuple (cdr inner)))))
...
(define (power relation k)
  (if (equal? k 0)
      '()
      (compPower relation relation k)))
(define (compPower relation power k)
  (if (equal? k 1)
      power
      (compPower relation(composite relation power)(- k 1))))
...
(define (transitive-closure relation)
  (tHelp relation relation(length relation)))
(define(tHelp r rel len)
  (if (equal? len 0)
      '()
      (if (equal? len 1)
          rel
          (union rel(tHelp r(power r len)(- len 1))))))
