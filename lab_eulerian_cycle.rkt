...
(define (maxVertex relation)
   (maxVertexH relation 1))

(define (maxVertexH relation k)
  (if (equal? '() relation)
      k
      (if (> (caar relation) k)
          (maxVertexH (cdr relation) (caar relation))
          (maxVertexH (cdr relation) k))))
...
(define (has-eulerian-cycle? relation)
   (h relation (maxVertex relation)))

(define (h relation vertex)
  (if (equal? 0 vertex)
      #t
